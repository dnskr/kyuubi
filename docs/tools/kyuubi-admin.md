...
This will result in the creation of a subdirectory named
apache-kyuubi-|release|-bin shown below,
...
The user defaults configs with key in format in the form of
...
The engine share level. If not specified, it will read the configuration
item kyuubi.engine.share.level from kyuubi-defaults.conf.

- -es, --engine-subdomain

The subdomain for the share level of an engine. If not specified, it will
read the configuration item kyuubi.engine.share.level.subdomain from
...
The proxy user to impersonate. When specified, it will delete engines for
the hs2ProxyUser.
