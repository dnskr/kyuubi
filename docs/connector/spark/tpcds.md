...
The TPC-DS is a decision support benchmark. It consists of a suite of business
...
> **Tip:** This article assumes that you have mastered the basic knowledge and
> operation of [TPC-DS](https://www.tpc.org/tpcds/). For the knowledge about
> TPC-DS not mentioned in this article, you can obtain it from its [Official
> Documentation](https://www.tpc.org/tpcds/).

This connector can be used to test the capabilities and query syntax of Spark
without configuring access to an external data source. When you query a TPC-DS
table, the connector generates the data on the fly using a deterministic
algorithm.
...
To enable the integration of Kyuubi Spark SQL engine and TPC-DS through Spark
DataSource V2 API, you need to:

- Referencing the TPC-DS connector [dependencies](#tpcds-dependencies)
- Setting the Spark catalog [configurations](#tpcds-configurations)
...
3. kyuubi-spark-connector-tpcds-|release|_2.12.jar, which can be found in the
   [Maven Central](https://repo1.maven.org/maven2/org/apache/kyuubi/kyuubi-spark-connector-tpcds_2.12/)

In order to make the TPC-DS connector package visible for the runtime classpath
...
To add TPC-DS tables as a catalog, we can set the following configurations in
