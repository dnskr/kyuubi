...
Architecture on top of existing storage systems such as S3, ADLS, GCS, and
HDFS.

```{tip}
This article assumes that you have mastered the basic knowledge and operation
of [Delta Lake](https://delta.io/). For the knowledge about Delta Lake not
...
- Setting the Spark extension and catalog
  [configurations](#spark-delta-lake-conf)

(spark-delta-lake-deps)=

### Dependencies

The **classpath** of Kyuubi Spark SQL engine with Delta Lake supported consists
of:

1. `kyuubi-spark-sql-engine-<release>_2.12.jar`, the engine jar deployed with a
   Kyuubi distribution
2. A copy of Spark distribution
3. `delta-core` & `delta-storage`, which can be found in the
   [Maven Central](https://mvnrepository.com/artifact/io.delta/delta-core)

In order to make the delta packages visible for the runtime classpath of
...
As for end-users, who only use a pure SQL interface, there aren't much
differences between using a delta table and a regular hive table. Unless you are
going to use some advanced features, but they are still SQL, just more syntax
added.
