...
After logging in with your Microsoft Azure account, please follow the steps below to
...
Download spark package that matches your environment from [spark official
website](https://spark.apache.org/downloads.html). And then unpackage:
...
Add following configuration to spark-defaults.conf, please refer to your own local
configuration for specific personalized configuration:
...
Create a new file named `core-site.xml` under `$SPARK_HOME/conf` directory, and add
...
Copy jar packages required by delta lake and microsoft azure to ./spark/jars
directory:
...
After this, you can check your data on azure web UI. For example, my container name
is 1000 and table name is alexDemo20211127:
...
If there is no problem with the above, it proves that spark has been built with delta
lake.
...
1. Download the latest version of kyuubi from [kyuubi download
...
Check kyuubi log, in order to check kyuubi start status and find the jdbc connection
url:
...
ZooKeeper, Kyuubi server can get the connection info from ZooKeeper and establish the
connection to the engine. So, you can check the registration details in zookeeper path
'/kyuubi_USER/anonymous'.
