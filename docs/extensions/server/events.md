...
# Configure Custom EventHandler

Kyuubi provide event processing mechanism, it can help us to record some
events. Beside the builtin `JsonLoggingEventHandler`, Kyuubi supports custom
event handler. It is usually used to write Kyuubi events to some external
systems. For example, Kafka, ElasticSearch, etc. The
`org.apache.kyuubi.events.handler.CustomEventHandlerProvider` has a zero-arg
constructor, it can help us to create a custom EventHandler.
...
To create custom EventHandlerProvider class derived from the above interface,
we need to:
...
- Adding a file named `org.apache.kyuubi.events.handler.CustomEventHandlerProvider`
